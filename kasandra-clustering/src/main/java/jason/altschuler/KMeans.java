...
public class KMeans {
    ...
    private List<KMeansVector> points;    // n-dimensional data points.
    ...
    // output
    private List<KMeansVector> centroids; // position vectors of centroids                      dim(2): (k) by (number of channels)
    ...
    public static class Builder {
        // required
        private final int k;
        private final List<KMeansVector> points;
         ...
         */
        public Builder(int k, List<KMeansVector> points) {
            ...
            HashSet<KMeansVector> hashSet = new HashSet<>(k);
            int distinct = 0;

            for (int i = 0; i < points.size(); i++) {
                if (!hashSet.contains(points.get(i))) {
                    distinct++;
                    if (distinct >= k)
                        break;
    ...
    private void run() {
        ...
        List<KMeansVector> bestCentroids = new ArrayList<KMeansVector>();
    ...
    private void assignmentStep() {
        ...
        for (int i = 0; i < m; i++) {
            ...
            for (int j = 0; j < k; j++) {
                tempDist = distance(points.get(i), centroids.get(i));
    ...
    private void updateStep() {
        ...
        for (int i = 0; i < k; i++)
            for (int j = 0; j < n; j++)
                centroids.get(i).getVector().add(j, 0.0);
        ...
        for (int i = 0; i < m; i++) {
            ...
            for (int j = 0; j < n; j++) {
                Double val = centroids.get(assignment[i]).getVector().get(j);
                centroids.get(assignment[i]).getVector().add(j, val + points.get(i).getVector().get(j));
            }
        }
        ...
        for (int i = 0; i < k; i++) {
            if (clustSize[i] == 0)
                ...
                for (int j = 0; j < n; j++) {
                    Double val = centroids.get(assignment[i]).getVector().get(j);
                    centroids.get(assignment[i]).getVector().add(j, val / clustSize[i]);
                }
        }

        // gracefully handle empty clusters by assigning to that centroid an unused data point
        if (emptyCentroids.size() != 0) {
            HashSet<KMeansVector> nonemptyCentroids = new HashSet<>(k - emptyCentroids.size());
            for (int i = 0; i < k; i++)
                if (!emptyCentroids.contains(i))
                    nonemptyCentroids.add(centroids.get(i));

            Random r = new Random();
            for (int i : emptyCentroids)
                while (true) {
                    int rand = r.nextInt(points.size());
                    if (!nonemptyCentroids.contains(points.get(rand))) {
                        nonemptyCentroids.add(points.get(rand));
                        centroids.set(i, points.get(rand));
                        break;
                    }
                }
    ...
    private void basicRandSample() {
        ...
        for (int i = 0; i < k; i++) {
            ...
            for (int j = 0; j < n; j++) {
                centroids.get(i).getVector().add(i, copy.get(rand).getVector().get(j));       // store chosen centroid
                copy.get(rand).getVector().add(j, copy.get(m - 1 - i).getVector().get(j)); // ensure sampling without replacement
            }
        }
    }
        ...
        for (int c = 0; c < k; c++) {
            ...
            if (c == 0)
                ...
                for (int p = 0; p < m; p++) {
                    ...
                    double tempDistance = Distance.L2(points.get(p), centroids.get(c - 1)); // need L2 norm here, not L1
            ...
            for (int i = 0; i < n; i++)
                centroids.get(c).getVector().add(i, points.get(choose).getVector().get(i));
        }
    }
     ...
     * @return
     */
    private double distance(KMeansVector x, KMeansVector y) {
    ...
    private static class Distance {
         ...
         * @return
         */
        public static double L1(KMeansVector x, KMeansVector y) {
            if (x.getVector().size() != y.getVector().size()) throw new IllegalArgumentException("dimension error");
            double dist = 0;
            for (int i = 0; i < x.getVector().size(); i++)
                dist += Math.abs(x.getVector().get(i) - y.getVector().get(i));
            return dist;
        }
         ...
         * @return
         */
        public static double L2(KMeansVector x, KMeansVector y) {
            if (x.getVector().size() != y.getVector().size()) throw new IllegalArgumentException("dimension error");
            double dist = 0;
            for (int i = 0; i < x.getVector().size(); i++)
                dist += Math.abs((x.getVector().get(i) - y.getVector().get(i)) * (x.getVector().get(i) - y.getVector().get(i)));
            return dist;
        }
    }
    ...
    private void calcWCSS() {
        ...
        for (int i = 0; i < m; i++) {
            ...
            WCSS += distance(points.get(i), centroids.get(assignedClust));
        }
    ...
    public List<KMeansVector> getCentroids() {
        return centroids;
    }
