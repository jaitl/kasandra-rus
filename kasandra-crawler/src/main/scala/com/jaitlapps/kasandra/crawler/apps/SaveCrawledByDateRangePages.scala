...
object SaveCrawledByDateRangePages extends App with StrictLogging with JsonExtension {
  ...
  val startTimestamp = new Timestamp(1451595600000L)
  val endTimestamp = new Timestamp(1483217999000L)

  val countFuture = crawledSitePagesDao.count(startTimestamp, endTimestamp)
  ...
  val pageFlow = Flow[Int]
    .mapAsync(1) { page =>
      ...
      crawledSitePagesDao.list(offset, limit, startTimestamp, endTimestamp)
    }
