...
class WallDispatcherActor(
  wallCrawlerActorCreator: ActorCreator[(ActorRef, CrawlSite)]
)
  extends Actor
  with ActorLogging {
  ...
  override def receive: Receive = {
    case StartCrawling(sites) =>
      ...
      sites.foreach { site =>
        ...
        val vkWallCrawler = wallCrawlerActorCreator.create(context, WallCrawlerActor.name(site))((self, crawlSite))
...
object WallDispatcherActor {
  ...
  def props(wallCrawlerActorCreator: ActorCreator[(ActorRef, CrawlSite)]): Props =
