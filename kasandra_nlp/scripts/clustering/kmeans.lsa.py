...
log_path = '/data/logs/%s.kmeans.lsa.logs' % year
...
logging.info("Start LSA n_topics: %s ..." % n_topics)
...
logging.info("End LSA time: %s ..." % (time.time() - start_lsa))

result_path = result_base + '/%s.%s.kmeans.lsa.json' % (year, n_clusters)
